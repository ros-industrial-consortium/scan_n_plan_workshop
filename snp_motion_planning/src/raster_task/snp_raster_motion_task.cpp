...
RasterFtMotionTask::RasterFtMotionTask(
    ...
    std::function<tesseract_planning::TaskComposerNode::UPtr(std::string)> raster_task_gen, bool is_conditional,
    std::string name)
  : tesseract_planning::TaskComposerTask(is_conditional, std::move(name))
{
  if (freespace_task_gen == nullptr)
  {
    freespace_task_gen = [](std::string description) {
      return std::make_unique<tesseract_planning::FreespaceMotionPipelineTask>(description);
    };
  }
  if (transition_task_gen == nullptr)
  {
    transition_task_gen = [](std::string description) {
      return std::make_unique<tesseract_planning::FreespaceMotionPipelineTask>(description);
    };
  }
  if (raster_task_gen == nullptr)
  {
    raster_task_gen = [](std::string description) {
      return std::make_unique<tesseract_planning::CartesianMotionPipelineTask>(description);
    };
  }
...
RasterFtMotionTask::runImpl(tesseract_planning::TaskComposerInput& input,
                            tesseract_planning::TaskComposerTask::OptionalTaskComposerExecutor executor) const
{
  ...
  for (std::size_t idx = 2; idx < program.size() - 2; idx += 2)
  {
    ...
    auto transition_mux_task = std::make_unique<tesseract_planning::UpdateStartAndEndStateTask>(
        prev.second, next.second, transition_pipeline_key, false);
  ...
  auto update_start_state_task = std::make_unique<tesseract_planning::UpdateStartStateTask>(raster_tasks.back().second,
                                                                                            to_end_pipeline_key, false);
  ...
  program.emplace_back(
      input.data_storage.getData(from_start_pipeline_key).as<tesseract_planning::CompositeInstruction>());
  for (std::size_t i = 0; i < raster_tasks.size(); ++i)
  {
    tesseract_planning::CompositeInstruction segment =
        input.data_storage.getData(raster_tasks[i].second).as<tesseract_planning::CompositeInstruction>();
    ...
    if (i < raster_tasks.size() - 1)
    {
      tesseract_planning::CompositeInstruction transition =
          input.data_storage.getData(transition_keys[i]).as<tesseract_planning::CompositeInstruction>();
      transition.erase(transition.begin());
      program.emplace_back(transition);
    }
  }
  tesseract_planning::CompositeInstruction to_end =
      input.data_storage.getData(to_end_pipeline_key).as<tesseract_planning::CompositeInstruction>();
