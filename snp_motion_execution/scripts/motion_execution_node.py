...
class MotionExecServer(Node):
    def __init__(self):
        ...
        self.joint_state_sub = self.create_subscription(JointState, self.JOINT_STATES_TOPIC, self.joint_state_cb, 10,
                                                        callback_group=self.cb_group)
        self.fjt_client = ActionClient(self, FollowJointTrajectory, fjt_action,
                                       callback_group=self.cb_group)
        self.enable_client = self.create_client(Trigger, self.ENABLE_SERVICE, callback_group=self.cb_group)
        self.server = self.create_service(ExecuteMotionPlan, self.MOTION_EXEC_SERVICE,
                                          callback=self.execute_motion_plan, callback_group=self.cb_group_server)

        self.get_logger().info("Motion execution node started")
    ...
    def execute_motion_plan(self, request: ExecuteMotionPlan.Request,
        ...
        try:
            ...
            if not self.fjt_client.wait_for_server(1.0):
